...
public sealed class DirectoryBackup : IDisposable
{
  ...
  public DirectoryBackup(string directoryPath)
  {
    ...
    CopyDirectoryContentsExceptBackup(_originalDirectoryPath,
  ...
  public void Dispose()
  {
    ...
    CopyDirectoryContentsExceptBackup(_backupDirectoryPath,
      _originalDirectoryPath);
    Directory.Delete(_backupDirectoryPath, true);
    Log.Information(
      "Restore original directory content and remove temporary backup directory");
  }

  public static DirectoryBackup? BackupDirectoryIfNecessary(
    string directoryPath, bool necessary)
  {
    if (!necessary) return null;
    Log.Information("Temporary backup directory: {BackupDirectory}",
      directoryPath);
